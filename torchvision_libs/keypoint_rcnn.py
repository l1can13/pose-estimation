...
def keypointrcnn_fpn(pretrained=False, progress=True,
                     num_classes=2, num_keypoints=17,
                     pretrained_backbone=True,
                     box_detections_per_img=100,
                     backbone=None):
    """
    Constructs a Keypoint R-CNN model with a flexible FPN backbone.
    """
    if pretrained:
        # no need to download the backbone if pretrained is set
        pretrained_backbone = False

    # Если backbone не передан, используем ResNet-50
    if backbone is None:

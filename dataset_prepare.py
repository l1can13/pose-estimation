...
dataset_headers = ['image']
coords = ['x', 'y', 'v']
for elem in keypointsMapping:
    for coord in coords:
        dataset_headers.append(elem + '_' + coord)
...
for img in os.listdir(path):
    ...
    for part in range(nPoints):
        ...
        dataset[keypointsMapping[part]] = keypoints
        keypoints_with_id = []
    ...
    result = []
    for values in zip_longest(*detected_keypoints):
        row = []
        for v in values:
            if v is None:
                row.extend([None] * 3)
